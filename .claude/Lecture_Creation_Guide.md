...
- **검토 중간**: `_review_step{N}.md` (구성안: N=1~4, 교안: N=1~6)
...
**교안 (`/lecture-script`)** — 6영역 45항목, 가중치 미적용:

| 영역 | 항목 수 | 검증 내용 |
|------|--------|----------|
| S: 구조 완전성 | 7 | §1~§8 존재, 필수 테이블, 22개 서브섹션, 차시 커버리지 |
| G: 교수설계 프레임워크 | 8 | Gagne ≥7/9, GRR 순차, 2-레이어 분리, Think-Aloud ≥3/4, 15분 분절 |
| P: 발문·평가·흐름 | 7 | Bloom's 점진 상승, CMU 3점(Exit 필수), 차시 간 전환 |
| T: 시간 배분 | 8 | 교시 합산, time_ratio ±5%, GRR 비율, 시간큐 연속성 |
| C: 콘텐츠 정확성 | 8 | Anti-Hallucination, CLO/SLO·차시·발문·활동 입력 원본 대조 |
| N: 교안 실행 품질 | 7 | 발화문 자연성, 활동 3요소, 집계 정합, 강사 가이드, 표기법 |
