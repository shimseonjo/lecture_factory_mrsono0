...
1. 4개 파일을 순서대로 Read:
   ...
   - `input_data.json` → required/optional 필드 존재 확인 (output_scope (Q13), lab_environment (Q14) 포함)
...
3. **교시별 상세 가이드**: 각 교시에 대해 50분 내부 구조를 적용
   ...
   - **필요 자료**: 슬라이드, 핸즈아웃, 소프트웨어 목록 (lab_environment가 있으면 해당 도구/환경 명시)
...
- **§9-2 환경 설정 트러블슈팅**: lab_environment가 있으면 해당 환경의 설치/설정 트러블슈팅을 정리. research_deep §3-2에 환경 설정 관련 소재가 있으면 통합
  - lab_environment가 없고 소재도 없으면 "해당 없음"으로 표기
- **§9-3 활동 운영 팁**:
...
| 9 | context7_block_{block_id}.md | 블록별 정밀 기술 문서·코드 예제 (블록 모드 기술 교육 시) |
| 10 | input_data.json tone_examples | 비유/메타포 |
| 11 | context7_reference.md | 기술 문서·코드 예제 (기술 교육 시) |
...
| I Do 시범 (Gagne 4~5) | **핵심 개념 설명** + 비유/메타포 전개 + 예시-비예시 쌍 + Think-Aloud 4대 메타인지. brainstorm §4의 설명전략을 발화문으로 풀어쓴다. 강사가 화면을 보며 설명하는 내용을 그대로 적는다 |
| We Do 안내 (Gagne 5~6) | 절차 안내 + 각 단계별 강사 코멘트 + 예상 실수 대응. 학습자와 함께 진행하는 과정을 대화체로 작성 |
| You Do (Gagne 6~7) | 과제 안내 + 성공 기준 설명 + 순회 피드백 시 강사 발화 예시 |
